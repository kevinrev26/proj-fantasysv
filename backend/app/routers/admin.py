...
def submit_match_results(
    ...
    # Try to enqueue the scoring task via Celery.
    # If the broker is unavailable, fall back to running it synchronously so
    # the admin always gets a result instead of a 500.
    try:
        ...
        # Broker is down — run scoring inline and return immediately.
        matchday.task_id = None
        matchday.task_status = "processing"
        db.add(matchday)
        db.commit()

        try:
            recalculate_matchday_scores_task.run(matchday_id)
        except Exception as exc:
            matchday.task_status = "error"
            db.add(matchday)
            db.commit()
            raise HTTPException(
                status_code=500,
                detail=f"Broker unavailable and synchronous scoring failed: {exc}",
            )

        return {"task_id": None, "status": "completed_synchronously"}
...
def get_matchdays(season_id: int, db: Session = Depends(get_db)):
    matchdays = (
        db.query(models.Matchday).filter(models.Matchday.season_id == season_id).all()
    )
...
def get_teams_for_season(season_id: int, db: Session = Depends(get_db)):
    ...
    return {
        "teams": [{"id": t.id, "name": t.name, "league_id": t.league_id} for t in teams]
    }
